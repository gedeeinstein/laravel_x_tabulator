...
var DayGridEventRenderer = /** @class */ (function (_super) {
    ...
    DayGridEventRenderer.prototype.fgSegHtml = function (seg, disableResizing) {
        ...
        var titleTxt = '';
        if(eventDef.titleUseHTML != '') {
            titleTxt = eventDef.titleUseHTML;
        } else if(eventDef.title != '') {
            titleTxt = (util_1.htmlEscape(eventDef.titleUseHTML || '') || '&nbsp;');
        }
        titleHtml =
            '<span class="fc-title">' +
                titleTxt + // we always want one line of height
                '</span>';
        return '<a class="' + classes.join(' ') + '"' +
            (eventDef.url ?
                ' href="' + util_1.htmlEscape(eventDef.url) + '"' :
